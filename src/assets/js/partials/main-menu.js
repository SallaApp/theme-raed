...
class NavigationMenu extends HTMLElement {
    ...
    */
    ...
    #hasChildren(menu = { children: [] }) { return menu?.children?.length > 0 }

    #hasProducts(menu = { products: [] }) { return menu?.products?.length > 0 }

    #clx(classes) { return `"${classes}"` }
    ...
    #getMobileMenu(menu = { children: [] }, displayAllText = this.#displayAllText) {
        const menuImage = menu.image ? /*html*/`<img src=${menu.image} class="rounded-full" width="48" height="48" alt=${this.#getMenuTitle(menu)} /> ` : ''

        return /*html*/`
        <li class="lg:hidden text-sm font-bold" ${menu.attrs}>
             ${!this.#hasChildren(menu) ? /*html*/`
             <a  ${menu.link_attrs} aria-label=${this.#getMenuTitle(menu) ? this.#getMenuTitle(menu) : 'category'} href=${menu.url} class=${this.#clx(`text-gray-500 ${menu.image ? '!py-3' : ''}`)}>
                    ${menuImage}
    ...
    <li class=${this.#getDesktopClasses(menu, isRootMenu)} ${menu.attrs}>
        <a ${menu.link_attrs} href=${menu.url} aria-label=${this.#getMenuTitle(menu) ? this.#getMenuTitle(menu) : 'category'}>
            <span>${this.#getMenuTitle(menu)}</span>
        </a>
    ...
    render() { this.innerHTML = this.#getHeaderMenu(this.#menus) }
}
