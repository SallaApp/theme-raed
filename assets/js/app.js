...
class App extends salla.AppHelpers {
    ...
    loadTheApp() {
        ...
        // reload the page each time the user add a comment in page or product
        salla.comment.event.onAdded(() => window.location.reload());

        // initiate update cart bag
        ...
        // initiate add to cart animation
    ...
    initiateAdAlert() {
        ...
        let storageKey = 'advertisement-' + ad.dataset.id;

        if (!salla.storage.get(storageKey)) {
            ad.classList.remove('hidden');
        }

        this.onClick('.ad-close', function (event) {
            event.preventDefault();
            salla.storage.set(storageKey, 'dismissed');
    ...
    initiateDropdowns() {
        // todo :: find a better way or avoid register the click event inside on click
